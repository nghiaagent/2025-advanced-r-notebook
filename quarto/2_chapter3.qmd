...
More details are available in the [Vectors Quiz](https://adv-r.hadley.nz/vectors-chap.html#quiz) book chapter.
...
```{webr Ex 3.1.1A}
#| autorun: true
# Build and show types of each vector
list(
  "logical" = c(TRUE, FALSE),
  "double" = c(1.41, 5.45),
  "integer" = c(1L, 2L),
  "character" = c("banana", "apple")
) |>
  map(typeof)
```
...
```{webr Ex 3.1.4B}
#| autorun: true
# Define dataframe with matrix column
x <- data.frame(names = c("a", "b", "c"))
x$val <- matrix(c(1:15), nrow = 3, ncol = 5)

# Show structure of this dataframe
str(x)

# Note that rendering of this dataframe is inconsistent between table viewers
message("Printing with print()")
print(x)

message("Printing with reactable()")
reactable::reactable(x)

message("Printing with datatable()")
DT::datatable(x)
...
More details are available in the [Atomic vectors](https://adv-r.hadley.nz/vectors-chap.html#atomic-vectors) book chapter.
...
# Show vector and type
message("Showing content of c(as.raw(40), as.raw(50))")
x
typeof(x)
...
# Show vector and type
message("Showing type of as.raw(c(40, 50))")
x
typeof(x)
...
# Show vector and type
message("Showing content of raw(2)")
x
typeof(x)
```
...
# Show vector and type
message("Showing content of c(1 + 6i, 1.7 + 3.2i)")
x
typeof(x)
...
## Show vector and type after coercion
message("Showing type of as.complex(c(1, 1.7))")
x
typeof(x)
...
## Show vector and type
message("Showing content of complex(2)")
x
typeof(x)
```
...
In order, these vectors will be coerced to their most basic type:
- Coerced to double
...
#| autorun: true
# Coerced to double
message("Showing type of c(1, FALSE)")
c(1, FALSE) |>
  typeof()

# Coerced to character
message("Showing type of c(\"a\", 1)")
c("a", 1) |>
  typeof()

# Coerced to integer
message("Showing type of c(TRUE, 1L)")
c(TRUE, 1L) |>
  typeof()
```
...
```{webr Ex 3.2.3A}
# First comparison: Both sides coerced to character prior to comparison.
c(1, "1") |>
  typeof()

message("Printing c(1, "1")")
c(1, "1") |>
  print()

# Second comparison: Both sides coerced to numeric prior to comparison.
c(-1, FALSE) |>
  typeof()

message("Printing c(-1, FALSE)")
c(-1, FALSE) |>
  print()
...
More details are available in the [Attributes](https://adv-r.hadley.nz/vectors-chap.html#attributes) book chapter.
...
More details are available in the [S3 atomic vectors](https://adv-r.hadley.nz/vectors-chap.html#s3-atomic-vectors) book chapter.
...
More details are available in the [Lists](https://adv-r.hadley.nz/vectors-chap.html#lists) book chapter.
...
More details are available in the [Data frames and tibbles](https://adv-r.hadley.nz/vectors-chap.html#tibble) book chapter.
